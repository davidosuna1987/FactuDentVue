...
@section('title')
	| Factura {{$invoice->invoice_no}} de {{$clinic->name}}
@endsection
...
@section('content')
	...
	<div id="invoice">
		...
		<div class="columns">
			<div class="column">
				<a href="{{route('clinics.invoices.edit', [$clinic, $invoice->invoice_no])}}" class="button is-primary is-pulled-right">Editar factura</a>
  			<a href="{{route('clinics.invoices', $clinic)}}" class="button m-r-10 is-pulled-right">Volver</a>
				<div class="title">Factura {{$invoice->invoice_no}} de {{$clinic->name}}</div>
				<hr>
			</div>
		</div>
