...
@section('content')
	...
	<div class="columns">
		<div class="column">
			<a href="{{route('clinics.edit', $clinic)}}" class="button is-primary is-pulled-right is-rounded">Editar clínica</a>
			<a href="{{route('clinics.invoices.create', $clinic)}}" class="button is-link m-r-10 is-pulled-right is-rounded">Crear factura</a>
  		<a href="{{route('clinics.index')}}" class="button m-r-10 is-pulled-right is-rounded">Volver</a>
			<div class="title title-secondary has-text-white">{{$clinic->name}}</div>
			{{-- <hr> --}}
		</div>
	</div>
